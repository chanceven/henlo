...
class _FurrentBoardingScreenState extends State<FurrentBoardingScreen> {
  ...
  final List<String> tabs = ['All', 'Pet Hotel', 'Home Boarding', 'Top Rated'];
...
Widget _buildServiceCard(Map<String, dynamic> service) {
  ...
  final lat = pawtner?['location_lat'] != null ? (pawtner!['location_lat'] as num).toDouble() : 0.0;
  final long = pawtner?['location_long'] != null ? (pawtner!['location_long'] as num).toDouble() : 0.0;

  final subtypeRaw = (service['service_subtype'] ?? '').toString();
  final subtypesList = subtypeRaw.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
  ...
  // Find all matching services for mini cards
  final matchingServices = services.where((s) {
    ...
    return s['pawtners']?['id'] == pawtnerId && (search.isEmpty || sName.contains(search) || pName.contains(search));
  }).toList();

  final showMiniCards = search.isNotEmpty && matchingServices.any((s) => (s['service_name'] ?? '').toString().toLowerCase().contains(search));
      ...
      builder: (context, snapshot) {
        ...
        if (search.isNotEmpty && businessName.toLowerCase().contains(search)) {
          ...
          while ((index = lowerName.indexOf(search, start)) != -1) {
            ...
            businessNameSpans.add(TextSpan(
              ...
              style: GoogleFonts.dosis(fontSize: 16, fontWeight: FontWeight.w600, color: const Color(0xFFFDCB58)),
            ));
            start = index + search.length;
          }
        ...
        return GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => FurrentPawtnerDetailScreen(
                pawtnerId: pawtnerId,                
                initialServiceId: service['id'].toString(),)),
            );
          },
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(12),
                        color: const Color(0xFF6E4B3A),
                        image: profileUrl != null ? DecorationImage(image: NetworkImage(profileUrl), fit: BoxFit.cover) : null,
                      ),
                      child: profileUrl == null ? const Icon(Icons.person, color: Color(0xFFDDC7A9), size: 32) : null,
                    ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Expanded(child: RichText(text: TextSpan(children: businessNameSpans))),
                              Row(
                                children: [
                                  const Icon(Icons.star, color: Color(0xFF6E4B3A), size: 16),
                                  const SizedBox(width: 2),
                                  Text(rating.toStringAsFixed(1), style: GoogleFonts.dosis(fontSize: 14, fontWeight: FontWeight.w600, color: const Color(0xFF6E4B3A))),
                                ],
                              ),
                            ],
                          ),
                          const SizedBox(height: 4),
                          Text('$businessAddress • ${distanceKm.toInt()} km', style: GoogleFonts.dosis(fontSize: 14, color: const Color(0xFF6E4B3A))),
                          const SizedBox(height: 4),
                          if (selectedTab == 'All' || selectedTab == 'Top Rated') Text(subtype, style: GoogleFonts.dosis(fontSize: 14, color: const Color(0xFF6E4B3A))),
                          const SizedBox(height: 4),
                          if (showMiniCards) ...matchingServices.map((s) => _buildServiceMiniCard(s, pawtnerId)).toList(),
                        ],
                      ),
                    ),
                  ],
  ...
  Widget build(BuildContext context) {
    // Filter unique pawtners to avoid duplicates
