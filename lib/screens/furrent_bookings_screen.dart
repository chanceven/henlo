...
class _FurrentBookingsScreenState extends State<FurrentBookingsScreen> {
  ...
  Future<void> _loadBookings() async {
    try {
      ...
      past.sort((a, b) {
        final aTime =
            DateTime.tryParse(a['scheduled_start'] ?? '') ?? DateTime.now();
        final bTime =
            DateTime.tryParse(b['scheduled_start'] ?? '') ?? DateTime.now();
  ...
  Widget build(BuildContext context) {
    ...
    final bookingsToShow = selectedPetId == null
        ? baseBookings
        : baseBookings
            .where((b) => b['pet_id']?.toString() == selectedPetId)
            .toList();
