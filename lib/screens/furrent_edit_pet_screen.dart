...
class FurrentEditPetScreen extends StatefulWidget {
  ...
  const FurrentEditPetScreen({super.key, required this.petData});

  @override
  State<FurrentEditPetScreen> createState() => _FurrentEditPetScreenState();
}

class _FurrentEditPetScreenState extends State<FurrentEditPetScreen> {
  ...
  void initState() {
    ...
    _petType = widget.petData['type'];
    _gender = widget.petData['gender'];

    if (widget.petData['birth_date'] != null) {
      DateTime birthDate = DateTime.parse(widget.petData['birth_date']);
      _selectedMonth = months[birthDate.month - 1];
  ...
  Future<void> _updatePet() async {
    ...
    try {
      ...
      await supabase
          .from('pets')
          .update(petData)
          .eq('id', widget.petData['id']);

      if (mounted) {
        _showToast("Pet updated successfully!");
        Navigator.pop(context);
      }
    } catch (e) {
      debugPrint('Error updating pet: $e');
      if (mounted) {
        _showToast("Failed to update pet");
      }
    } finally {
          ...
          children: [
            ...
            Expanded(
              child: _petImageBytes != null
                  ...
                  : widget.petData['profile_picture_url'] != null &&
