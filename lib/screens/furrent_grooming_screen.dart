...
class FurrentGroomingScreen extends StatefulWidget {
  const FurrentGroomingScreen({super.key});

  @override
  State<FurrentGroomingScreen> createState() => _FurrentGroomingScreenState();
}

class _FurrentGroomingScreenState extends State<FurrentGroomingScreen> {
  ...
  final List<String> tabs = ['All', 'Pet Shop', 'Home Service', 'Top Rated'];
  ...
  Future<void> _loadServices() async {
    ...
    try {
      var query = supabase
          ...
          .ilike('service_type', 'grooming')
          .eq('is_public', true);

      if (selectedTab == 'Pet Shop') {
        query = query.ilike('service_subtype', '%pet shop%');
      } else if (selectedTab == 'Home Service') {
        query = query.ilike('service_subtype', '%home service%');
      }
      ...
      debugPrint('Error loading grooming services: $e');
  ...
  Widget _buildServiceCard(Map<String, dynamic> service) {
    ...
    bool hasPetShop = subtypesList
        .map((s) => s.toLowerCase())
        .any((s) => s.contains('pet shop'));
    ...
    if (selectedTab == 'Home Service') {
      locationText = 'See All Locations';
    } else if (subtypesList.length == 1 &&
        subtypesList.first.toLowerCase() == 'home service') {
      locationText = 'See All Locations';
    } else if (hasPetShop) {
      if (lat != 0 && long != 0) {
                  ...
                  : Padding(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          // Get unique pawtner IDs
                          final uniquePawtnerIds = services
                              .map((s) => s['pawtners']?['id'])
                              .toSet()
                              .toList();

                          final pawtnerId = uniquePawtnerIds[index];

                          // Get all services for this pawtner
                          final pawtnerServices = services
                              .where((s) => s['pawtners']?['id'] == pawtnerId)
                              .toList();

                          // Pass the first service to _buildServiceCard
                          return _buildServiceCard(pawtnerServices.first);
                        },
                      ),
                    ),
            )
